...
def signin():
    ...
    if Username == "admin" and Password == "1234":
        screen = Tk()
        screen.geometry("1275x720+0+0") 
        screen.resizable(True, True)
        screen.title("Stelliva Coffee Shop")  #nama aplikasi

        topFrame = Frame(screen, bd=10, relief=RIDGE, bg="white")  #frame judul
        topFrame.pack(side=TOP)

        labelTitle = Label(
            topFrame,
            text="STELLIVA",
            font=("Castellar", 39, "bold"),
            fg="white",
            bg="#53B0FC",
            bd=15,
            width=10,
        ...
        def show():
            metode = Label (inputframe, text= dropdown.get())
            metode.pack()
        
        metode_list = ["Tunai", "QRIS", "Transfer"]
        dropdown = StringVar()
        dropdown.set(metode_list[0])  # Pilih tunai secara default
        dropdown_menu = OptionMenu(inputframe, dropdown, *metode_list)
        ...
        inputframeadmin.place(x=10, y=90)
        ...
        def struk():
            ...
            if thsa:
                ...
                bayar_entry.get() !="Rp 0"
                bayar_pelanggan = int(bayar_entry.get())  # Mendapatkan nilai pembayaran dari input
                kembalian_pelanggan = kembalian2.cget("text")  # Mendapatkan nilai kembalian dari label

                textStruk.insert(END, f"Bayar\t\t\tRp. {bayar_pelanggan}\n\n")
        ...
        menuFrame = Frame(screen, bd=10, relief=RIDGE, bg="#42AAFF")
        menuFrame.pack(side=LEFT)

        hargaFrame = Frame(menuFrame, bd=9, relief=RIDGE, bg="#2196F3", pady=12)
        hargaFrame.pack(side=BOTTOM)

        bayarFrame = Frame(menuFrame, bd=9, relief=RIDGE, bg="#2196F3", pady=12)
        bayarFrame.pack(side=RIGHT)

        brunchFrame = LabelFrame(
            menuFrame,
            text=" Food ",
            font=("Castellar", 19, "bold"),
            bd=10,
            relief=RIDGE,
            fg="#2f2f2f",
            bg="#f6f6f6",
        ...
        rightFrame = Frame(screen, bd=15, relief=RIDGE)
        ...
        textStruk = Text(
            strukFrame, font=("arial", 12, "bold"), bd=3, width=36, height=26
        )
