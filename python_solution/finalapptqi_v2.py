...
def process_csv(file_path, selected_headers, divider, lines, firstvalue, startpoint, endpoint,encoding):
    # Fungsi generate_pairs harus didefinisikan sebelumnya
    # pairs = generate_pairs(selected_headers)

    try:
        start_row, rows_to_read = automatevalidrow(file_path, selected_headers,encoding)

        rows_to_skip  = [0] + [i for i in range(start_row, start_row+rows_to_read)]

        data = pd.read_csv(file_path , sep=';', encoding=encoding, skiprows=rows_to_skip)
        ...
        def loopcheck(data, startloop):
            ...
            dmtocm = data.iloc[startloop]['Km'] * 100000  # Ambil nilai Km dan konversi ke cm
            firstvalue = int(dmtocm / 1000)  # Ambil bagian ribuan
            meter_start = int(dmtocm - firstvalue * 1000)  # Sisanya dalam meter
            checkvalue = int(data.iloc[startloop]['Km']* 10000) % 10  # Ambil digit terakhir dari 'Km'
            return firstvalue, meter_start, checkvalue,startloop
        ...
        if firstvalue == "":
            try:
                firstvalue, meter_start, checkvalue,startloop = loopcheck(data, startloop)
        ...
        rows_to_skip  = [0] + [i for i in range(start_row+startloop+1, start_row+rows_to_read-startloop-1)]

        data = pd.read_csv(file_path , sep=';', encoding=encoding, skiprows=rows_to_skip)

        # Pengecekan jika data mengandung string tertentu
        ...
        with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
            ...
            summary_df['Meter Awal'] = summary_df.index * divider / 4 + firstvalue*1000 + meter_start
