...
##update 5 -> menangani jika posisi start tidak berada di 0.25
...
def process_excel(file_path, selected_headers, divider, lines, firstvalue, startpoint, endpoint):
    ...
    try:
        ...
        data = pd.read_excel(
            file_path, 
            sheet_name=0, 
            skiprows=range(1, start_row+1),  # Melewati baris sebelum start_row
        ...
        def loopcheck(data, startloop):
            ...
            dmtocm = data.iloc[startloop]['Km'] / 10  # Ambil nilai Km dan konversi ke cm
            firstvalue = int(dmtocm / 1000)  # Ambil bagian ribuan
            meter_start = int(dmtocm - firstvalue * 1000)  # Sisanya dalam meter
            checkvalue = int(data.iloc[startloop]['Km']) % 10  # Ambil digit terakhir dari 'Km'
        ...
        data = pd.read_excel(
            file_path, 
            sheet_name=0, 
            skiprows=range(1, start_row +1 + startloop),  # Melewati baris sebelum start_row
            nrows=rows_to_read - startloop # Hanya membaca jumlah baris yang relevan
        )
