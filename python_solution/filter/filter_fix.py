            ...
            try:
                ...
                for sheet_name in excel_file.sheet_names:
                    ...
                    if action == "Hapus Baris 1-4":
                        ...
                        # Buat nama file CSV berdasarkan nama file dan sheet
                        base_filename = os.path.splitext(file)[0]
                        output_path = os.path.join(output_folder, f"{base_filename}_{sheet_name}.csv")
                        
                        # Simpan sebagai CSV dengan header
                        df.to_csv(output_path, index=False, header=True)
                        print(f"Berhasil memproses sheet {sheet_name} dari {file} -> Disimpan sebagai {output_path}")
                    else:
                        # Untuk aksi lain (misalnya transpose), tetap simpan sebagai XLSX
                        output_xlsx_path = os.path.join(output_folder, file)
