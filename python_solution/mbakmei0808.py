...
if 'C' in df.columns:
    # Membatasi setiap nilai di kolom 'C' menjadi 12 karakter pertama
    df['C'] = df['C'].astype(str).str[:12]
...
if 'C' in df.columns:
    ...
    # Menambahkan kolom "Status" untuk menandai duplikat
    df_pi['Status'] = df_pi.duplicated(subset='C', keep=False).replace({True: 'Duplicat', False: ''})
    df_fc['Status'] = df_fc.duplicated(subset='C', keep=False).replace({True: 'Duplicat', False: ''})
    df_fp['Status'] = df_fp.duplicated(subset='C', keep=False).replace({True: 'Duplicat', False: ''})
    df_td['Status'] = df_td.duplicated(subset='C', keep=False).replace({True: 'Duplicat', False: ''})
    df_pt['Status'] = df_pt.duplicated(subset='C', keep=False).replace({True: 'Duplicat', False: ''})
    df_lainnya['Status'] = df_lainnya.duplicated(subset='C', keep=False).replace({True: 'Duplicat', False: ''})
