...
def process_excel(file_path, selected_headers, divider, lines, firstvalue, startpoint, endpoint,pairs):
    ...
    try:
        ...
        if not set(selected_headers).issubset(data.columns):
        ...
        with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
            ...
            for idx, header in enumerate(selected_headers):
                # Nama sheet berdasarkan header kolom
                sheet_name_raw = header
            ...
            # Menentukan parameter yang tidak berpasangan secara otomatis
            paired_parameters = [param for pair in pairs for param in pair]  # Mengambil semua parameter dari pasangan
            remaining_params = [param for param in parameters if param not in paired_parameters]  # Sisa parameter yang tidak berpasangan
            ...
            for idx in range(len(summary_df)):
                ...
                # Menghitung total untuk pasangan parameter berdasarkan nama
                for param1_name, param2_name in pairs:
                    # Mencari indeks parameter berdasarkan nama
                    param1_idx = parameters.index(param1_name)
                    param2_idx = parameters.index(param2_name)
                    
                    avg_pair = (summary_df.iloc[idx, param1_idx] + summary_df.iloc[idx, param2_idx]) / 2
                    total_kai += avg_pair
                    total_en += avg_pair**2

                # Menghitung total untuk sisa parameter yang tidak berpasangan
                for param_name in remaining_params:
                    param_idx = parameters.index(param_name)
                    total_kai += summary_df.iloc[idx, param_idx]
                    total_en += summary_df.iloc[idx, param_idx]**2

                # Menyimpan hasil perhitungan dengan pembulatan
...
while True:
    ...
    if event == "Preview Header":
        ...
        if not file_path:
            ...
            try:
                # Membaca sheet pertama dan menampilkan header
                data = pd.read_excel(file_path, sheet_name=0)
    ...
    if event == "Proses":
        ...
        if not file_path or not selected_headers:
            ...
            pairs = [('LPROF_MID', 'RPROF_MID'), ('LALIGN_MID', 'RALIGN_MID')]  # Nama parameter yang berpasangan
            result = process_excel(file_path, selected_headers, divider, lines, firstvalue, startpoint, endpoint,pairs)
