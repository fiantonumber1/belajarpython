...
def process_csv(file_path, selected_headers, divider, firstvalue, encoding,idinput):
    ...
    try:
        ...
        with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
            ...
            awalrute = summary_df['Km Awal'] * 1000 + summary_df['Meter Awal']
            akhirrute = summary_df['Km Akhir'] * 1000 + summary_df['Meter Akhir']
            summary_df['NilaiTengah'] = (awalrute + akhirrute) / 2
            ...
            for index, row in summary_df.iterrows():
                ...
                for kota in datakota:
                    ...
                    if lower_bound <= row['NilaiTengah'] <= upper_bound:
                        ...
                        summary_df.loc[index, 'Rute'] = kota["nama"]
                        kota_terdeteksi = True
                        kotadilewati.append(kota["nama"])
                        break  # Jika kota ditemukan, hentikan pencarian lebih lanjut

                # Jika tidak ada kota yang terdeteksi, set "Kota tidak terdefinis"
                if not kota_terdeteksi:
                    summary_df.loc[index, 'Rute'] = "Kota tidak terdefinis"
            ...
            summary_df_2['Rute'] = summary_df['Rute']
            ...
            for i in range(len(indexname)):
                ...
                rows = []
                for kota in datakota:
                    # Memfilter DataFrame berdasarkan rute
                    filtered_df = summary_df[summary_df['Rute'] == kota["nama"]]
                    
                    # Jika DataFrame kosong, lewati iterasi ini
                    if filtered_df.empty:
                        print(f"Rute untuk kota {kota['nama']} tidak ditemukan, melewati iterasi.")
                        continue
